...
def main():
    ...
    encoded_string_1 = "BAMV TAQJB TA TFAMA TAJJ"
    encoded_string_2 = "WAJABQ AF MATAQ ATAJAM"
    encoded_string_3 = "FAQTAQX TA BA QATQATTAM"
    encoded_string_4 = "A BATAJXQT AF MATAQ AMJAAQFAB"

    # Set of all unique words in the encoded strings
    words = set(encoded_string_1.split() + 
                encoded_string_2.split() + 
                encoded_string_3.split() + 
                encoded_string_4.split())

    wordlist_paths = ['en_short_wordlist.txt', 'en_long_wordlist.txt', 'dragonkin_wordlist.txt']
    ...
    for wordlist_path in wordlist_paths:
        ...
        with open(f'decoded_results_{wordlist_path.split("_")[0]}.json', 'w') as json_file:
